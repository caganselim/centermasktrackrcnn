...
class EncoderCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MASKGNN.MASK_FEATURE_SRC == "logits":
        
            input_dim = cfg.MODEL.FCOS.NUM_CLASSES
            linear_in = (pool_dim)*(pool_dim)
            
        else:

            input_dim = cfg.MODEL.MATCHER.OBJ_ENCODER.ENCODER_CNN.INPUT_DIM # 256
            linear_in = (pool_dim // 2) *(pool_dim // 2)
                       
        hidden_dim = cfg.MODEL.MATCHER.OBJ_ENCODER.ENCODER_CNN.HIDDEN_DIM
        output_dim = cfg.MODEL.MATCHER.OBJ_ENCODER.ENCODER_CNN.OUTPUT_DIM
