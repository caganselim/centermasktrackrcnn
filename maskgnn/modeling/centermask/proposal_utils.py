...
def add_ground_truth_to_proposals_single_image(targets_i, proposals):
    ...
    if proposals.has('pred_deltas'):

        if targets_i.has('gt_deltas'):
            gt_proposal.pred_deltas = targets_i.gt_deltas
            # proposals.deltas = proposals.pred_deltas
            # #proposals.remove("pred_deltas")
        else:
            gt_proposal.pred_deltas = torch.zeros(0, 4).cuda()

    """
    Problem is here.
    """

    # print("proposals: ", proposals , "\n \n gt_proposal: ", gt_proposal)
